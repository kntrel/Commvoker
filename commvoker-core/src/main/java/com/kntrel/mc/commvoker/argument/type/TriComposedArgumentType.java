...
public interface TriComposedArgumentType<S, A, B, C, T> extends ComposedArgumentType<S, T> {

    Producer<A> firstDelegate(ArgumentContext ctx);
    Producer<B> secondDelegate(ArgumentContext ctx);
    Producer<C> thirdDelegate(ArgumentContext ctx);

    T compose(CommandContext<S> ctx, A first, B second, C third);
    ...
    default boolean thirdSuggests() {
        return TypeUtils.hasMethod(this.getClass(), "getSuggestionsThird", CommandContext.class, SuggestionsBuilder.class);
    }
    ...
    default CompletableFuture<Suggestions> getSuggestionsThird(CommandContext<S> ctx, SuggestionsBuilder suggestionsBuilder) {
        return new CompletableFuture<>();
    }
    ...
    default Collection<Pair<Producer<?>, SuggestionProvider<S>>> delegates(ArgumentContext ctx) {
        SuggestionProvider<S> firstProvider = this::getSuggestionsFirst,
                              ...
                              thirdProvider = this::getSuggestionsThird;
        return List.of(
                ...
                new SimplePair<>(this.secondDelegate(ctx), this.secondSuggests() ? thirdProvider : null)
        );
    }
    @Override @SuppressWarnings("unchecked")
    default T contextualize(CommandContext<S> context, Object[] subject) {
        return this.compose(context, (A) subject[0], (B) subject[1], (C) subject[2]);
    }
}
